...
object Anagrams {
  implicit def ordering[Char]: Ordering[Char] = Ordering.fromLessThan(_.toString < _.toString)

  /** A word is simply a `String`. */
  def main(args: Array[String]) {
    val x: List[Char] = List('a', 'd', 'b')
    //    x.sorted foreach print
    val s = List("abcd", "e")
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy(x => x).toList.sorted.map(x => (x._1, x._2.length))
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    x.foldLeft(List[(Char, Int)]())((a, b) =>
    y.filter(e => e._1 == b._1) match {
      case List() => a :+ (b._1, b._2)
      case (ch, b._2) :: tail => a
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =  {
    def helper(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case xs => for {
        ...
        tail <- helper(subtract(occurrences, combinations))
      } yield word :: tail
    }

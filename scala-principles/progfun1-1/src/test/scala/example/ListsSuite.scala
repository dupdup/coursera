 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of number list with negative numbers") {
    assert(sum(List(3, -1, 0, 2)) === 4)
  }

  test("max of a few numbers") {
    assert(max(List(3, 7, 2)) === 7)
    assert(max(List(3, 3, 2)) === 3)
    assert(max(List(2, 3, 2)) === 3)
  }
  ...
  test("max of numbers has negative numbers") {
    assert(max(List(3,-1)) === 3)
    assert(max(List(-1,3)) === 3)
  }

  test("throw NoSuchElementException on empty") {
